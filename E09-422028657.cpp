...
int main()
{
	...
	//ALAS
	alaup_M = Model();
	alaup_M.LoadModel("Models/alaup.obj");
	aladown_M = Model();
	aladown_M.LoadModel("Models/aladown.obj");

	//puerta
	puerta_M = Model();
	puerta_M.LoadModel("Models/PUERTA_ARCO.obj");
	...
	float rotala = 0.0f; //angulo de rotación del ala
	float velala = 1.2f; //velocidad de aleteo
	int alamov = 0; //alas hacia arriba o hacia abajo
	int avanza = 1; //carro avanza o no
	float unoydos = 0.0f; //contador para tiempo entre 1 y 2
	float tiempoacu = 0.0f; //contador para tiempo entre numeros
	...
	while (!mainWindow.getShouldClose())
	{
		...
		model = glm::mat4(1.0);
		model = glm::translate(model, glm::vec3(0.0f - angulovaria * 0.01, 5.0f + sin(glm::radians(angulovaria)), 6.0));
		...
		//der
		//Material_brillante.UseMaterial(uniformSpecularIntensity, uniformShininess);
		model = glm::translate(model, glm::vec3(-0.5f, 0.35f, 3.0f));
		...
		*/
		...
		color = glm::vec3(1.0f, 1.0f, 1.0f);
		glUniform3fv(uniformColor, 1, glm::value_ptr(color));
		//acumula tiempo para hacer cambio
		unoydos += deltaTime * 0.01;
		//cambios
		if (unoydos <= 1) {
			if (unoydos >= 0 && unoydos < 0.5) {
				Numero1Texture.UseTexture();
			}
			else if (unoydos >= 0.5 && unoydos <= 1) {
